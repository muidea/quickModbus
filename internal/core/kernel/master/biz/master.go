...
func (s *mbSerialMaster) reset() {
	s.signalGard.Reset()
	s.tcpClient = nil
}

func (s *mbSerialMaster) connect(serverAddr string) (ret tcp.Client, err error) {
...
func (s *mbSerialMaster) Start(serverAddr string) (err error) {
	connClient, connErr := s.connect(serverAddr)
	if connErr != nil {
		err = connErr
		log.Errorf("start master %s failed, error:%s", serverAddr, connErr.Error())
...
func (s *mbSerialMaster) IsConnect() bool {
	return s.tcpClient != nil
}

func (s *mbSerialMaster) ReConnect() (err error) {
	connClient, connErr := s.connect(s.serverAddr)
	if connErr != nil {
		err = connErr
		log.Errorf("reconnect master %s failed, error:%s", s.serverAddr, connErr.Error())
...
func (s *mbSerialMaster) OnConnect(ep tcp.Endpoint) {
...
func (s *mbSerialMaster) OnDisConnect(ep tcp.Endpoint) {
	log.Warnf("onDisConnect from %s", ep.RemoteAddr().String())
	s.reset()
}

func (s *mbSerialMaster) OnRecvData(ep tcp.Endpoint, data []byte) {
	dataVal := bytes.NewBuffer(data)
...
func (s *mbSerialMaster) ReadCoils(address, count uint16) (retData []byte, exCode byte, err error) {
	protocol := model.NewReadCoilsReq(address, count)
...
func (s *mbSerialMaster) ReadDiscreteInputs(address, count uint16) (retData []byte, exCode byte, err error) {
	protocol := model.NewReadDiscreteInputsReq(address, count)
	header := model.NewSerialHeader(s.address)
...
func (s *mbSerialMaster) ReadHoldingRegisters(address, count uint16) (retData []byte, exCode byte, err error) {
	protocol := model.NewReadHoldingRegistersReq(address, count)
	header := model.NewSerialHeader(s.address)
...
func (s *mbSerialMaster) ReadInputRegisters(address, count uint16) (retData []byte, exCode byte, err error) {
	protocol := model.NewReadInputRegistersReq(address, count)
	header := model.NewSerialHeader(s.address)
...
func (s *mbSerialMaster) WriteSingleCoil(address uint16, data []byte) (retAddr uint16, retData []byte, exCode byte, err error) {
	protocol := model.NewWriteSingleCoilReq(address, data)
	header := model.NewSerialHeader(s.address)
...
func (s *mbSerialMaster) WriteMultipleCoils(address, count uint16, data []byte) (retAddr, retCount uint16, exCode byte, err error) {
	protocol := model.NewWriteMultipleCoilsReq(address, count, data)
	header := model.NewSerialHeader(s.address)
...
func (s *mbSerialMaster) WriteSingleRegister(address uint16, data []byte) (retAddr uint16, retData []byte, exCode byte, err error) {
	protocol := model.NewWriteSingleRegisterReq(address, data)
	header := model.NewSerialHeader(s.address)
...
func (s *mbSerialMaster) WriteMultipleRegisters(address, count uint16, data []byte) (retAddr, retCount uint16, exCode byte, err error) {
	protocol := model.NewWriteMultipleRegistersReq(address, count, data)
	header := model.NewSerialHeader(s.address)
...
func (s *mbSerialMaster) ReadExceptionStatus() (retStatus, exCode byte, err error) {
	protocol := model.NewReadExceptionStatusReq()
	header := model.NewSerialHeader(s.address)
...
func (s *mbSerialMaster) Diagnostics(subFuncCode uint16, data []byte) (retSubFuncCode uint16, retData []byte, exCode byte, err error) {
	protocol := model.NewDiagnosticsReq(subFuncCode, data)
	header := model.NewSerialHeader(s.address)
...
func (s *mbSerialMaster) GetCommEventCounter() (status uint16, eventCount uint16, exCode byte, err error) {
	protocol := model.NewGetCommEventCounterReq()
	header := model.NewSerialHeader(s.address)
...
func (s *mbSerialMaster) GetCommEventLog() (status uint16, eventCount, messageCount uint16, events []byte, exCode byte, err error) {
	protocol := model.NewGetCommEventLogReq()
	header := model.NewSerialHeader(s.address)
...
func (s *mbSerialMaster) ReportSlaveID() (ret []byte, exCode byte, err error) {
	protocol := model.NewReportSlaveIDReq()
	header := model.NewSerialHeader(s.address)
...
func (s *mbSerialMaster) ReadFileRecord(items []*common.ReadItem) (ret [][]byte, exCode byte, err error) {
	reqItems := []*model.ReadRequestItem{}
	for _, val := range items {
		reqItems = append(reqItems, model.NewReadRequestItem(val.FileNumber, val.RecordNumber, val.RecordLength))
	}
...
func (s *mbSerialMaster) WriteFileRecord(items []*common.WriteItem) (exCode byte, err error) {
	reqItems := []*model.WriteItem{}
	for _, val := range items {
		byteVal, byteErr := hex.DecodeString(val.RecordData)
		if byteErr != nil {
...
func (s *mbSerialMaster) MaskWriteRegister(address uint16, andBytes []byte, orBytes []byte) (retAddr uint16, retAnd []byte, retOr []byte, exCode byte, err error) {
	protocol := model.NewMaskWriteRegisterReq(address, andBytes, orBytes)
	header := model.NewSerialHeader(s.address)
...
func (s *mbSerialMaster) ReadWriteMultipleRegisters(readAddr, readCount uint16, writeAddr, writeCount uint16, writeData []byte) (retData []byte, exCode byte, err error) {
	protocol := model.NewReadWriteMultipleRegistersReq(readAddr, readCount, writeAddr, writeCount, writeData)
	header := model.NewSerialHeader(s.address)
...
func (s *mbSerialMaster) ReadFIFOQueue(address uint16) (retDataCount uint16, retDataVal []byte, exCode byte, err error) {
	protocol := model.NewReadFIFOQueueReq(address)
	header := model.NewSerialHeader(s.address)
